...
class MixedClassifier(torch.nn.Module):
    ...
    def forward(self, event_time, event_type, TS, padding_masks, statics=None):
        # Ensure padding_masks has correct dimensions
        if padding_masks.dim() == 1:
            padding_masks = padding_masks.unsqueeze(1).expand(-1, TS.size(1))  # Expand to [batch_size, sequence_length]
