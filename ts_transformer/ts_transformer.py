...
class TSTransformerEncoder(nn.Module):
    ...
    def forward(self, X, padding_masks):
        ...
        # Ensure padding_masks has the correct shape
        if len(padding_masks.shape) == 1:
            padding_masks = padding_masks.unsqueeze(1).repeat(1, X.size(1))

        # Permute input for transformer
        inp = X.permute(1, 0, 2)
        ...
        output = output.permute(1, 0, 2)  # Restore (batch_size, seq_length, d_model)
