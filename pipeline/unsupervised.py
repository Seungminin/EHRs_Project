...
def L1_L2_L3_pretrain(model, loader, writer, learning_rate, record_freq, downstream_freq=10, total_epoch=100, finetune=False, downstream_model=None, downstream_loaders=None):
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    ...
    for epoch in (pbar := tqdm(range(total_epoch), desc="Training Epochs")):
        for batch in loader:
            x, padding_mask, event_times, event_types, targets, statics = map(lambda x: x.cuda(), batch)
            x = x.float().transpose(2, 1)  # Transpose for (batch_size, seq_length, feature_dim)

            # Generate random masks
            ts_masks = torch.from_numpy(
                np.random.choice([True, False], size=x.shape, replace=True, p=(masking_ratio, 1 - masking_ratio))
            ).cuda()

            # Ensure padding_mask is expanded to match sequence length
            if len(padding_mask.shape) == 1:
                padding_mask = padding_mask.unsqueeze(-1).repeat(1, x.size(1))

            # Initialize forecast_mask
            forecast_mask = torch.zeros(x.size(0), x.size(1), device='cuda')  # (batch_size, seq_length)

            # Calculate valid lengths
            leng = torch.sum(padding_mask, dim=-1).long() - 1
            leng = torch.clamp(leng, min=0, max=x.size(1) - 1)  # Ensure within valid range

            # Scatter lengths into forecast_mask
            forecast_mask = forecast_mask.scatter_(1, leng.unsqueeze(1), 1)
