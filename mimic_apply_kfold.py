...
if task == 'Supervised':
    ...
    #Continuous한 데이터 시계열 데이터에 초점을 둔 방법
            ...
            # K-Fold 데이터 로드
            loaders_per_fold, test_loader = prepare_balanced_loaders_from_csv_kfold(downs_dataname, batch_size=batch_size, max_ts_len=150, max_event_len=400)
            
            count = 0
            # 각 Fold에 대해 학습
            for fold_idx, (train_loader, val_loader) in enumerate(loaders_per_fold):
                print(f"\nStarting Fold {fold_idx + 1}/{len(loaders_per_fold)}")
                ...
                print(f"Shape (TS): {ts_shape}, Shape (Static): {static_shape}")
                ...
                print(f"Fold {fold_idx + 1}: Train, Val, Test Sizes = {[len(loader) for loader in loaders]}")

                # Fine-tuning with the current Fold
                val_scores, test_scores, step = mixed_finetune_balanced_graph(
                    downs_model, loaders, None, lr, torch.nn.CrossEntropyLoss(), 2, 400,count = count
                )

                print(f"Fold {fold_idx + 1}: Validation Scores: {val_scores}, Test Scores: {test_scores}")
                count+=1
